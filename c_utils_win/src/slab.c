...
int slab_init(slab_t* slab, size_t slab_size, uint8_t* blob, size_t blob_size) {
  slab->size = ROUND_UP(slab_size, sizeof(void*)) + sizeof(struct slab_unit);

  if (slab->size > blob_size) return -1;
...
int slab_free(void* block) {
  ...
  if (p->canary != 0xdeadbeaf) return -1;
