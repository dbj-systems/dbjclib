...
int _test_str_sep_do(char *input, char *sep, int toks_len, char *toks[],
                     bool outcome) {
  int len = 0;
...
#define _test_sep_pos(i, s, t) \
  if (_test_str_sep_do(i, s, ARRAY_SIZEOF(t), t, true)) break;
#define _test_sep_neg(i, s, t) \
  if (_test_str_sep_do(i, s, ARRAY_SIZEOF(t), t, false) == 0) break;
int test_str_sep() {
  int retval = -1;
...
int test_str_sep_count() {
  if (str_sep_count("1,2,3,4,5", ",") != 5) return -1;
  if (str_sep_count("   1 2,  3,,,,4,5   ", ", ") != 5) return -1;
  if (str_sep_count("1,  2,,,3  ,4    ,5 ", ", ") != 5) return -1;
  if (str_sep_count(",,,,,,  1 2 3 4 5", " ,") != 5) return -1;
  if (str_sep_count("1 2 3 4 5", ",") != 1) return -1;
  return 0;
}
