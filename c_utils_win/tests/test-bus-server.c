...
int bus_write_check(int *fd) {
  ...
  if (ret != TEST_MSG_LEN) {
    printf("write %d failed!", write_fd);
    return -1;
  }

  for (i = 0; i < NUM_CLIENTS; i++) {
    if (i == write_fd) continue;
    ret = read(fd[i], buf, sizeof(buf));
    if (ret != TEST_MSG_LEN) {
      printf("read %d failed!", i);
      return -1;
    }
    if (strncmp(buf, TEST_MSG, TEST_MSG_LEN)) {
      printf("msg check %d failed!", i);
      return -1;
    }
  }
...
int test_bus_server() {
  ...
  for (i = 0; i < NUM_CLIENTS; i++) {
    ...
    if (fd[i] < 0) {
      printf("connect %d failed!", i);
      return -1;
    }
  }

  for (i = 0; i < 10; i++) {
    if ((rc = bus_write_check(fd))) break;
  }

  for (i = 0; i < NUM_CLIENTS; i++) close(fd[i]);
...
TEST_DEF(bus_server) {
  ...
  if (rc < 0) {
    printf("Failed to start bus server");
    return;
  }
  printf("bus server started");
