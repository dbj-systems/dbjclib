...
int test_dict_insert(hash_map_t *map, const char *const *words, size_t count) {
  ...
  printf("Inserted %zu items", ndx);
  return 0;
}

int test_dict_iterator(hash_map_t *map, const char *const *words,
                       size_t count) {
  ...
  HASH_MAP_FOREACH(map, &key, &val) {
    if (strcmp(val->key, words[val->off])) {
      mismatch += 1;
      printf("Err: word mismatch '%s/%s' off: %zu", words[ndx], val->key,
             val->off);
    }
    ndx++;
  }
  if (ndx != count || mismatch) return -1;
  printf("Iterated over %zu items", ndx);
  return 0;
}
...
int test_dict_delete(hash_map_t *map, const char *const *words, size_t count) {
  ...
  while (ndx < count) {
    ...
    if (p == NULL) {
      not_found += 1;
      printf("word '%s' not found", words[ndx]);
    } else {
      if (p->off != ndx || strcmp(p->key, words[ndx])) {
        not_found += 1;
        printf("Err: word '%s/%s' off: %zu", words[ndx], p->key, ndx);
      }
      free_test_hashmap_value(NULL, p);
    }
    ndx++;
  }
  printf("Deleted %zu items", ndx);
  return not_found * -1;
}
